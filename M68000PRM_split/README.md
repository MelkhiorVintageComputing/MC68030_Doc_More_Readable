...
- [`INSTRUCTIONS-BY-CPU.md`](INSTRUCTIONS-BY-CPU.md) — which instructions each processor and coprocessor in the family supports, built from Appendix A's Table A-1 and cross-checked against the per-processor tables and the section 4/5/6 instruction headers; 10 documentation discrepancies are listed with the evidence. Also as [`instructions-by-cpu.csv`](instructions-by-cpu.csv), regenerated by [`make-instructions-by-cpu.py`](make-instructions-by-cpu.py) and verified by [`check-instructions-by-cpu.py`](check-instructions-by-cpu.py).
- [`CONDITION-CODES.md`](CONDITION-CODES.md) — what each of the 122 MC680xx instructions does to X, N, Z, V and C, in the manual's own notation but with every `*` replaced by a numbered note, so the 193 asterisk cells become 60 distinct explanations. Read out of the `Condition Codes:` block of each instruction description and cross-checked against Table 3-18 and against a second extraction of the same pages; 18 documentation defects are listed with the evidence. Also as [`condition-codes.csv`](condition-codes.csv), [`condition-code-notes.csv`](condition-code-notes.csv) and [`table-3-18-condition-code-computations.csv`](table-3-18-condition-code-computations.csv), regenerated by [`make-condition-codes.py`](make-condition-codes.py) and verified by [`check-condition-codes.py`](check-condition-codes.py).
