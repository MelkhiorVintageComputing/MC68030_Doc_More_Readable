...
Redrawn from **Figure 12-2** of the MC68881/MC68882 User's Manual (PDF page 405,
printed Foldout-1 — see [`21-timing-diagram-foldouts.pdf`](21-timing-diagram-foldouts.pdf)
for the original scan, which carries all three bus-cycle figures side by side on one sheet).

A complete asynchronous read of a coprocessor interface register. The address, `R/W` and `CS` are driven first, then `AS` and `DS` assert; `START` — the logical start-of-access condition — goes true; the FPCP answers with `DSACK1` and `DSACK0` and drives `D0-D31`; the strobes negate and the data bus is released.

![Figure 12-2 — Asynchronous Read Cycle Timing Diagram](figure-12-2-asynchronous-read-cycle.svg)

## Specifications marked on this figure

<!-- BEGIN TABLE from=ac-electrical-specifications.csv table=read-write nums=6|6A|7|7A|8|8A|9|9A|10|10A|11|11A|13|13A|14|15|16|19|19A|20|21|22 -->
...
| 14 | CS, DS Asserted to Data-Out Valid (Read) | ns | ≤ 80 | ≤ 45 | ≤ 45 | ≤ 30 |
...
```sh
python3 make-figure-svg.py 2      # redraw this figure
